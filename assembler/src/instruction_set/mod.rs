...
use crate::{
    util::{
        instruction_parsing::{parse_immediate, parse_label, parse_register},
        Labels, U12, U4,
    },
...
pub struct Instruction {
    ...
    opcode: U4, // flags: InstructionFlags,
}
...
impl Instruction {
    ...
    fn validate_parameters(&self, parameters: &str, labels: &Labels) -> AssemblyResult<u8> {
        match self.instruction_type() {
            InstructionType::R => Ok((parse_register(parameters)?.get() << 4) as u8),
            InstructionType::I => {
                // TODO: Parameters for Jump arguments
                parse_immediate(parameters)
            }
            InstructionType::J => parse_label(parameters, labels),
            InstructionType::B => match parameters {
                "" => Ok(0),
                other => Err(AssemblyError::InvalidParameter(
                    self.name.into(),
                    other.into(),
                )),
            },
