...
use crate::{AssemblyError, AssemblyResult, instruction_set::Instruction, util::{Labels, U12}};
    ...
    for i in 0..lines.len() {
        ...
        match first_word.split_once(":") {
            ...
            Some((label, rest)) => {
                ...
                if !rest.is_empty() || !terms.as_str().is_empty() {
...
pub fn parse_lines(lines: &Vec<&str>, labels: &Labels) -> AssemblyResult<Vec<U12>> {
    ...
    for i in 0..lines.len() {
        ...
        let instruction_str = if first_word.contains(":") {
            let remainder = first_word.split_once(":").ok_or(AssemblyError::UnreachableParsing)?.1;
            if remainder == "" {
                continue;
            } else {
                remainder
        ...
        instructions.push(instruction.gen_machine_code(&terms.as_str(), labels)?);
    }
